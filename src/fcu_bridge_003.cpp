...
int main(int argc, char **argv) {
  ...
  odom=nh.subscribe<nav_msgs::Odometry>("/vins_estimator/odometry_003", 100, odomHandler, ros::TransportHints().tcpNoDelay());
  cmd=nh.subscribe<std_msgs::Int16>("/fcu_bridge/command", 100, cmdHandler, ros::TransportHints().tcpNoDelay());
  mission=nh.subscribe<geometry_msgs::InertiaStamped>("/fcu_bridge/mission_003", 100, missionHandler, ros::TransportHints().tcpNoDelay());
