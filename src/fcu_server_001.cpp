			...
			if (mavlink_parse_char(chan,rbPop(&mav_buf_receive), &msg_received, &status)){
                ...
                switch (msg_received.msgid) {
                    ...
                    case MAVLINK_MSG_ID_SET_GPS_GLOBAL_ORIGIN:
                        ...
                        gnss_origin_pub.latitude=set_gps_global_origin.latitude*1e-7;//纬度:deg
                        gnss_origin_pub.longitude=set_gps_global_origin.longitude*1e-7;//经度:deg
                        gnss_origin_pub.altitude=set_gps_global_origin.altitude*1e-2;//海拔：cm->m
                        gnss_origin.publish(gnss_origin_pub);
                        break;
...
int main(int argc, char **argv) {
    ...
    ros::NodeHandle nh("~");
    nh.param("DRONE_PORT", PORT, 10001);
